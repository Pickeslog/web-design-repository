...
- 대시보드 마스코트. 크로비(따뜻함) / 롭(디스토피아 AI), 클릭 시 교감 말풍선(캐릭터별 톤) + 경험치. 생일 당일에는 클릭 시 축하 말풍선으로 전환.

상태:

- 기본 교감(클릭) / 연타 시 어지러움(dizzy, 0.9초 내 3연속 클릭 → 1.2초 후 복구) / 방치 20초 시 졸음(sleepy) + 8~15초 랜덤 혼잣말(mutter) / 드래그 들어올리기(롭 전용, 150px↑·3초 지속 시 scared → angry → drop)

### FriendshipLevelBar (우정 레벨 + 경험치)

구성:

- 레벨 배지 / 진행 바 (레벨에 따라 `--level-accent` 색 진화)
- XP 적립: 마스코트 교감(+2, 하루 3회) · 약속 등록(+3) · 약속 완료(+15) · 게시글 작성(기본+25, 사진 1장당+1 최대+10, 글자수 50↑+5/100↑+10 중 최고 1개)
- 누적 게시글 수 기반 가속 배율(30개→x1.2 / 50개→x1.5 / 70개→x2.0 / 100개→x3.0), 방치형 패시브 경험치("기억의 샘": 누적게시물×0.5 + 다가올일정×1)
- 100% 초과 시 연속 레벨업, 111단위 마일스톤(소규모 폭죽+컬러 진화), 만렙 777(6방향 대규모 폭죽), 만렙 이후 하트(7개)·별(12개) 파티클 특수 반응(경험치 없음)
