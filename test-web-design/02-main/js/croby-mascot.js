...
(function () {
  ...
  function pickReaction() {
    ...
    var isBirthdayEvent = false;
    if (typeof window.v5state !== 'undefined') {
      if (window.v5state.event === 'my_birthday') {
        pool = [
          { text: '오늘 생일 정말 축하해! 🎂' },
          { text: '태어나줘서 고마워! 행복한 하루 보내!' },
          { text: '오늘 맛있는 거 많이 먹어! 🎁' }
        ];
        isBirthdayEvent = true;
      } else if (window.v5state.event === 'friend_birthday') {
        pool = [
          { text: '오늘 친구 생일이네! 축하 메시지 남겨봐!' },
          { text: '생일인 친구에게 행운편지를 써볼까? 💌' },
          { text: '친구와 함께 즐거운 하루 보내길 바라! 🎉' }
        ];
        isBirthdayEvent = true;
      }
    }

    if (character === 'robot' && !isBirthdayEvent) {
  ...
  hit.addEventListener('click', function () {
    ...
    if (clickTimes.length >= CONFIG.clicksToDizzy && mode !== 'dizzy') {
      ...
      /* 한마디 랜덤 (로봇인 경우 find/pencil 리액션으로 스프라이트도 함께 바뀐다) */
      var reaction = pickReaction();
      window.lastMascotLine = reaction.text; // grantXP에서 쓰기 위해 저장
      ...
      try {
        if (typeof window.v5LevelUp === 'function') {
          ...
          showSay("levelUp 함수를 찾을 수 없습니다!", 5000);
        }
      } catch (e) {
        console.error(e);
      }
...
if(window.ClovMascot) { window.ClovMascot.playLevelUpEffect = function() { if(typeof spawnConfettiBurst === 'function') { const mascot = document.getElementById('croby'); if(mascot) spawnConfettiBurst(mascot, {count: 40, spread: 150}); } }; }
