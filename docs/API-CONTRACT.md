...
- **기록 보존** — 나가기=`status=LEFT`, 탈퇴=익명화(**§5-2**), 추억삭제=soft delete.
...
| DELETE | `/api/v1/users/me` | 계정 탈퇴 = **익명화**(`is_anonymized`·닉네임 "언노운"·이메일/인증정보 파기), **Access·Refresh 즉시 무효화**. 기록 FK 보존 (→ §5-2) | 본인 |
...
**DELETE `/users/me`** → `data: null` (익명화 — 상세는 **§5-2**)
...
### 5-2. 탈퇴 = 익명화 — 정확히 무엇이 지워지고 무엇이 남나

> 2026-08-12 정정(clov-api#157·#159). 그전까지 이 절은 "닉네임을 언노운으로 바꾸고 refresh를 revoke한다"까지만 적고 있었는데, **그 문장대로 구현했더니 탈퇴한 계정으로 로그인이 그대로 됐다.** 아래가 실제 동작이다.

`DELETE /users/me` 한 번에 서버가 하는 일:

| 대상 | 처리 | 이유 |
|---|---|---|
| `is_anonymized` · `anonymized_at` | `true` · 현재 시각 | 탈퇴 표식 |
| `nickname` | `'언노운'` | 화면 표기 |
| `email` | `anonymized-{id}@clov.invalid` | **유일값으로 치환**(`uk_users_email`이 UNIQUE라 원값을 두면 그 이메일이 영구히 묶인다). `.invalid`는 RFC 2606 예약 도메인이라 실제로 발송 가능한 주소가 될 수 없다 |
| `password` · `oauth_provider` · `oauth_subject` | `NULL` | 소셜 계정도 같은 방식으로 풀린다(`uk_users_oauth`는 NULL끼리 서로 다른 값으로 취급) |
| Refresh 토큰 | 전부 revoke | |
| **이미 발급된 Access 토큰** | **즉시 무효** | 아래 참조 |
| 추억·편지·사진·방 멤버십·레벨 | **그대로 보존** | §3 "기록 보존" |
| `birthdate` | row에는 남지만 **응답에서 차단** | `RoomMember.birthMonthDay`가 익명화 계정이면 `null`(§6) |

**로그인이 막히는 방식** — `AuthService.login()`·`CustomOAuth2UserService`에 `is_anonymized` 분기는 **없다.** `email`·`oauth_subject`가 비어서 `findByEmail`/`findByOauth`가 그 행을 못 찾고, 결과적으로 `401 INVALID_CREDENTIALS`가 난다. 즉 **이 차단은 위 컬럼 파기에 전적으로 의존한다** — 익명화 SQL에서 컬럼 하나라도 빼면 로그인이 조용히 다시 열린다.

**Access 토큰 즉시 차단** — 탈퇴 직전 발급된 액세스 토큰이 TTL(30분) 동안 room·shop·letter 등 거의 모든 API에 그대로 통했다(`is_anonymized`를 보는 `UserService.findUser()`를 안 거치는 경로). `JwtAuthenticationFilter`가 **매 요청 `users.is_anonymized`를 확인**해 `401 UNAUTHORIZED`로 끊는다. 프론트 인터셉터는 이 401에 `/auth/refresh`를 시도하지만 refresh가 이미 revoke돼 실패하고 그대로 로그아웃 처리된다 — **이것이 의도된 흐름이다**(§4-4가 401을 피한 이유와 달리, 여기서는 로그아웃이 목적이라 401이 맞다).

**같은 이메일·소셜 계정으로 재가입할 수 있다.** 새 `id`를 받는 **완전히 다른 사용자**이고, 옛 기록은 탈퇴한 옛 `id`에 그대로 남아 섞이지 않는다. 사용자 눈에는 "초기화"처럼 보이지만 기록 보존 원칙과 어긋나지 않는다.

⚠️ **화면 문구는 "삭제"라고 쓰면 안 된다.** 추억은 친구 공간에 남고 작성자 표기만 "언노운"이 된다. "모두 사라진다"고 안내하면 지우려는 사람은 안 지워지고 남기려는 사람은 겁나서 못 누른다(clov-web#165에서 실제로 걸러낸 문구). 현재 문구는 `AccountDeleteDialog.jsx` 참조.

⚠️ **2026-08-12 이전에 익명화된 행은 이 처리가 적용돼 있지 않다** — `email`·`password`가 원값 그대로다. Access 토큰 차단은 소급 적용되지만(필터가 `is_anonymized`만 본다), **그 이메일은 여전히 재가입이 막혀 있다.** 소급 정리는 별도 판단 사항이다.
...
| GET | `/api/v1/users/me/notifications/unread` | 안읽음 알림 존재 여부(방 목록 종 아이콘 배지 전용, 유저 전체 기준) | 로그인 |
| GET | `/api/v1/rooms/{roomId}/notifications/unread` | 안읽음 알림 존재 여부(방 안 종 아이콘 배지 전용, 이 방만 기준) | 본인 수신분 |

- 서버가 멤버 입·퇴장·방 설정 변경·가입 신청 등을 트리거로 생성(클라 생성 API 없음).
- **`GET /users/me/notifications/unread`는 room 무관, 유저 전체 기준이다.** 방 목록(홈) 화면 종 아이콘 배지 전용. `recipient_id` 기준으로 `is_read=false`인 행이 하나라도 있으면 `hasUnread: true`. 개수는 안 준다(배지는 점 하나면 충분, §13 최상단 changelog 참고).
- **`GET /rooms/{roomId}/notifications/unread`는 그 방만 기준이다(clov-api#174).** 방 안(Header variant=room) 화면 종 아이콘 배지 전용. 위 유저 전체 기준 엔드포인트와 쿼리만 다르고 응답 모양은 동일(`{hasUnread}`) — 다른 방에 안읽음이 남아 있어도 이 방을 전부 읽으면 여기선 `false`가 온다(전체 기준 배지와 값이 갈릴 수 있는 게 의도다). 인가는 목록 조회(`GET /rooms/{roomId}/notifications`)와 동일하게 방 ACTIVE 멤버만.
- **★ 이 방에 PENDING 가입 신청이 있으면 `hasUnread`는 무조건 `true`다(web-design-repository#98).** `notifications` 테이블과는 별도로 `room_join_requests WHERE room_id=? AND status='PENDING'` 존재 여부를 OR로 더한다 — `JOIN_REQUEST`를 알림으로 되살리는 게 아니다(그건 바로 아래 "JOIN 탭은 알림 테이블과 무관하다" 결정과 충돌한다). **읽음 처리(개별 클릭·읽음)는 이 조건에 영향을 주지 않는다** — 가입 신청을 승인하거나 거절해서 `PENDING`을 벗어나야만 이 배지 요인이 꺼진다. 정원 8명 제한·수락 후 5분 되돌리기처럼 시간이 걸리는 처리인데 종 아이콘이 아예 반응하지 않던 것(room/39에서 실측)을 메운다.
...
- **★ `MEMBER_JOINED`·`JOIN_ACCEPTED`도 `JOIN`이 아니라 `FRIEND`다 (리더 확정 2026-08-04, web-design-repository#51)** — `MEMBER_LEFT`를 `FRIEND`로 정할 때 세운 원칙("`type`은 처리할 게 있는 탭인지로 가른다")을 그대로 적용한다. `JOIN` 탭은 `Notifications.jsx`가 알림 테이블을 아예 조회하지 않는 **가입 신청 처리함**이다(`enabled: activeTab !== 'JOIN'`) — 정보성 알림을 거기 두면 사용자가 볼 방법이 없다. 실제로 `#112`/`#113`으로 두 subType을 만든 뒤에도 알림 화면 어디에도 안 뜨는 상태였다(소식 피드의 "최신 5건" 안에 남아 있을 때만 보임). `MEMBER_JOINED`와의 대칭 때문에 처음엔 `JOIN`이 맞아 보였지만, **대칭보다 "실제로 보이는 곳"을 다시 한번 우선했다** — `MEMBER_LEFT` 때와 같은 판단.
  ...
  - ⚠️ 이 "사문화" 결정은 **`notifications` 테이블에 `JOIN_REQUEST` 행을 쌓는 것**에 대한 것이다. 방 안 종 배지(`GET /rooms/{roomId}/notifications/unread`)가 `room_join_requests`를 직접 참조하는 것과는 별개다 — 위 §13 엔드포인트 절의 "PENDING 가입 신청" 항목(web-design-repository#98) 참고.
...
**GET `/rooms/{roomId}/notifications/unread`** → `{ "hasUnread": true }`
