...
| | |
|---|---|
| 카탈로그 전체(할인 반영) | **71,660 G** — 시드 12종 42,260 + 마스코트 스킨 8종 29,400 |
| 가장 싼 코스튬(클로버 배지) | 600 G |
| 시작 골드 | **1,000 G** |

> ⚠️ **이 총액은 스킨이 늘 때마다 커진다.** 42,260은 스킨이 하나도 없던 2026-07-30 값이었고, 8월 초에 스킨 8종이 들어오면서 70% 커졌다. **§15-4의 획득 속도를 조정할 때 이 숫자를 다시 세지 않으면 "며칠이면 다 산다"가 실제와 어긋난다.** 세는 쿼리는 아래와 같다.
>
> ```sql
...
> **속도 감각** — 시작 1,000 + 하루 최대 3,000. 가장 싼 코스튬 600G는 첫날 바로 살 수 있고, **카탈로그 전체(71,660)는 약 24일**이다. **구매 → 장착 → 마스코트 변경 루프를 하루 안에 여러 번 완주**시키는 값이다.
>
> 개정 전 상한 500으로는 같은 카탈로그가 **143일**이라 데모에서 상점이 사실상 잠겨 있었다(당시 표기 "85일"은 스킨 8종이 들어오기 전 카탈로그 42,260 기준이었다).
>
