...
> 최종 갱신: 2026-08-04 — **§15-1 카탈로그 총액을 계산값에서 실측값으로 교체**(clov-api #129) — `71,660`(계산)은 시드 쪽이 틀린 값이었고 실제는 **71,860**(시드 11종 37,060 + 스킨 12종 34,800, 23행)이다. 재는 쿼리를 **시드/스킨으로 쪼개도록** 바꿨고, **`code` 충돌이 기존 상품을 조용히 덮어쓴다**는 경고를 신설했다(등록 SQL의 검증 쿼리로는 안 잡힌다). 이전 — **§12 마스코트 교감 캡 3회 → 10회 + 응답에 `earnedGold` 명시**(clov-api #92) — §15-4를 10회로 올리면서 §12를 3으로 둬 **하루 2,000골드가 방 4개 이상 있어야 도달하는 죽은 값**이 됐던 것을 맞췄다. 두 캡은 같은 값이어야 하고 **스코프는 방 단위/유저 단위로 다르다**(양쪽에 상호 참조를 박아뒀다). 이전 — **§15-4 골드 획득 개정**(clov-api #92) — 마스코트 200/10회 · 약속 추억 300 · **자유 추억 `EARN_MEMORY_FREE` 200/3회 신설** · 총 상한 3,000. 금액이 노력 순서를 따르도록 뒤집었다(클릭 한 번이 약속 완료보다 비쌌다). 이전 — **§13 `MEMBER_JOINED`·`JOIN_ACCEPTED`를 `JOIN`→`FRIEND`로 이동**(web-design-repository#51) — `JOIN` 탭은 알림 테이블을 조회하지 않는 가입 신청 처리함이라 정보성 알림이 안 보이던 문제(`MEMBER_LEFT`와 같은 원인). 이전 — **§13 `MEMBER_LEFT` 신설**(clov-api #122) — 프로즈가 이미 약속하던 '퇴장'에 대응 행이 없던 것을 메웠다. 탭은 `FRIEND`(근거는 §13 해당 항목). 이전 2026-07-30 — **§15 Shop(상점)·Wallet 신설**(#14) + §14 상점 에러 6종 + §5 `equippedItem` + **시작 골드 1,000 확정**(구현값 20,000은 테스트 데이터에 맞춘 값이라 폐기). 이전 2026-07-20 — §2 `details`·§14 공통 6종(#3) + §4-1 auth 스키마·인증 에러코드·비번 정책(#6) + §4-2 소셜 토큰 전달(일회성 코드)·동의(#5 준비) + **§4-3 공통 읽기 모델·§5~§13 요청/응답·pagination 스키마 보강**(발명 위험 제거, M2 팬아웃 선행) + §4 소셜 콜백 `users` 생성 시점 정정(consent-선행). 이전 2026-07-14 — 구 계약(즉시입장·OAuth-only·SB3.5) 대체.
...
| | |
|---|---|
| 카탈로그 전체(할인 반영) | **71,860 G** — 시드 11종 37,060 + 마스코트 스킨 12종 34,800 (총 23행) |
| 가장 싼 코스튬(클로버 배지) | 600 G |
| 시작 골드 | **1,000 G** |

> **★ 위 세 숫자는 2026-08-04에 DB에서 직접 잰 값이다.** 계산으로 낸 값이 아니다.
>
> ⚠️ **이 총액은 스킨이 늘 때마다 바뀐다. 반드시 다시 재라 — 계산하지 말 것.** 직전까지 여기 적혀 있던 `71,660`(시드 12종 42,260 + 스킨 8종 29,400)은 **한 번도 실측된 적이 없는 계산값**이었고, 실제로 시드 쪽이 틀려 있었다. **§15-4의 획득 속도를 조정할 때 이 숫자를 잘못 들고 있으면 "며칠이면 다 산다"가 통째로 어긋난다.** 재는 쿼리는 아래와 같다.
>
> ```sql
...
>        SUM(ROUND(price * (100 - discount_rate) / 100)) AS catalog_total
> FROM shop_items;
> ```
>
> **총액만 재지 말고 시드/스킨으로 쪼개서 잴 것.** 총액 하나만 보면 어느 쪽이 틀렸는지 못 가린다 — 실제로 2026-08-04에 총액이 예상과 어긋났을 때, 이 쪼갠 값이 있어서 **"스킨 코드가 시드 코드와 충돌해 기존 상품을 덮어썼다"**는 원인까지 5분 만에 짚혔다(아래).
>
...
>
> ⚠️ 이 개정을 내릴 때 근거로 삼은 카탈로그 총액 `42,260 → 71,660`은 **계산값이었고 나중에 실측해보니 시드 쪽이 틀려 있었다**(실제 71,860). 방향은 맞았지만 **판단 근거가 잰 값이 아니었다** — §15-1의 재는 쿼리를 먼저 돌릴 것.
...
> **속도 감각** — 시작 1,000 + 하루 최대 3,000. 가장 싼 코스튬 600G는 첫날 바로 살 수 있고, **카탈로그 전체(71,860 실측)는 약 24일**이다. **구매 → 장착 → 마스코트 변경 루프를 하루 안에 여러 번 완주**시키는 값이다.
>
> 개정 전 상한 500으로는 같은 카탈로그가 **약 142일**이라 데모에서 상점이 사실상 잠겨 있었다.
>
