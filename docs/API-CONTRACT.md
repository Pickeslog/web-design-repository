...
> 최종 갱신: 2026-07-30 — **§15-4 골드 획득(`EARN_MASCOT`/`EARN_MEMORY`) 추가**(#16) + §15 Shop(상점)·Wallet 신설(#14) + §14 상점 에러 6종 + §5 `equippedItem` + **시작 골드 1,000 확정**(구현값 20,000은 테스트 데이터에 맞춘 값이라 폐기). 이전 2026-07-20 — §2 `details`·§14 공통 6종(#3) + §4-1 auth 스키마·인증 에러코드·비번 정책(#6) + §4-2 소셜 토큰 전달(일회성 코드)·동의(#5 준비) + **§4-3 공통 읽기 모델·§5~§13 요청/응답·pagination 스키마 보강**(발명 위험 제거, M2 팬아웃 선행) + §4 소셜 콜백 `users` 생성 시점 정정(consent-선행). 이전 2026-07-14 — 구 계약(즉시입장·OAuth-only·SB3.5) 대체.
...
| reason | 금액 | 발생 시점 | 제한 |
|---|---|---|---|
| `EARN_MASCOT` | +100 | 마스코트 교감 성공(`POST /rooms/{roomId}/mascot/interact`) | **유저 단위** 하루 3회 |
| `EARN_MEMORY` | +50 | **약속에 연결된** 추억 등록(`POST /plans/{planId}/memories`) | 일일 총 상한에만 걸린다 |

- **일일 총 상한 500골드.** 사유와 무관하게 한 유저가 하루에 받는 총액이다. 도달 후에는 **XP만 오르고 골드는 오르지 않는다.**
- **`EARN_MEMORY`는 FREE MEMORY(`plan_id` NULL, §10 D3)에 지급하지 않는다.** 두 가지 이유다 — FREE MEMORY는 약속·완료·다른 사람이 필요 없어 무한 반복이 가능하고, 보상은 서비스의 핵심 흐름(약속 완료 → 추억 작성)을 강화해야 한다.
- **캡은 유저 단위다. 방 단위로 잡지 않는다** — 한 사람이 가입할 수 있는 방 수에 상한이 없고 혼자서도 만들 수 있어서, 방 단위 캡은 하루 획득량을 방 개수에 비례해 늘리는 장치가 된다.
- **§12의 XP 캡과 독립이다.** 그래서 **같은 교감에서 XP는 오르고 골드는 안 오를 수 있다** — 프론트는 골드가 안 올랐다고 교감 실패로 판단하지 않는다.
- 지급은 XP 지급과 **같은 트랜잭션** 안에서 일어난다 — 하나만 반영되는 경우는 없다.
